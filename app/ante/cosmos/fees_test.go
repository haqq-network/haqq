	...
	testcases := []deductFeeDecoratorTestCase{
		...
		},
		{
			...
			malleate: func() {
				...
				err := testutil.FundAccountWithBaseDenom(ctx, appHaqq.BankKeeper, fgAddr, initBalance.Int64())
				suite.Require().NoError(err)
				// grant the fees
				grant := sdk.NewCoins(sdk.NewCoin(
					utils.BaseDenom, initBalance,
				))
				err = appHaqq.FeeGrantKeeper.GrantAllowance(ctx, fgAddr, addr, &feegrant.BasicAllowance{
					SpendLimit: grant,
				})
				suite.Require().NoError(err)
			},
		...
		},
		{
			...
			malleate: func() {
				...
				dfd = cosmosante.NewDeductFeeDecorator(
					appHaqq.AccountKeeper, appHaqq.BankKeeper, appHaqq.DistrKeeper, nil, appHaqq.StakingKeeper, nil,
				)
			},
		},
	}

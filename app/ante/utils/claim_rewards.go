...
func ClaimStakingRewardsIfNecessary(
	...
	if err != nil {
		return errorsmod.Wrap(err, "error while retrieving bond denom")
	}
...
func ClaimSufficientStakingRewards(
	...
	if errIter := stakingKeeper.IterateDelegations(
		cacheCtx,
		addr,
		...
		return errorsmod.Wrap(errIter, "error while iterating delegations")
	}

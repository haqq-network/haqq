...
func init() {
	...
	sdk.GetConfig().SetBech32PrefixForAccount("haqq", "")
}
...
func (suite *LedgerTestSuite) TestEvmosLedgerDerivation() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			derivationFunc := ledger.HaqqLedgerDerivation()
...
func (suite *LedgerTestSuite) TestGetAddressPubKeySECP256K1() {
	...
	addr := crypto.PubkeyToAddress(privKey.PublicKey)
	expAddr, err := sdk.Bech32ifyAddressBytes("haqq", common.HexToAddress(addr.String()).Bytes())
	suite.Require().NoError(err)

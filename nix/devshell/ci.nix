{ ... }:
{
  ...
  scripts.ci-proto.exec = ''
    ...
    # Check for changes, but exclude script files and other source files that shouldn't be generated
    # Only check files that should be generated by proto/swagger generation
    CHANGED_FILES=$(git diff --name-only | grep -E '\.(pb\.go|pb\.gw\.go|pulsar\.go)$|swagger.*\.(json|yaml)$|statik\.go$' || true)

...
import (
	...
	"github.com/haqq-network/haqq/precompiles/bank/testdata"
	...
	testutils "github.com/haqq-network/haqq/testutil/integration/evmos/utils"
...
func (is *IntegrationTestSuite) SetupTest() {
	...
	is.haqqAddr = common.HexToAddress(erc20precompile.WEVMOSContractTestnet)
	...
	genAccs := []authtypes.GenesisAccount{
		&haqqtypes.EthAccount{
			BaseAccount: authtypes.NewBaseAccount(is.haqqAddr.Bytes(), nil, 0, 0),
			CodeHash:    codeHash,
		},
		&haqqtypes.EthAccount{
...
var _ = Describe("Bank Extension -", func() {
	...
	Context("Direct precompile queries", func() {
		...
		Context("supplyOf query", func() {
			It("should return the supply of Evmos", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(directCall, contractData, bank.SupplyOfMethod, is.haqqAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")
	...
	Context("Calls from a contract", func() {
		...
		Context("supplyOf query", func() {
			It("should return the supply of Evmos", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(contractCall, contractData, SupplyOfFunction, is.haqqAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")

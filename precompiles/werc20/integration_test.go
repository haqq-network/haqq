...
import (
	...
	"github.com/haqq-network/haqq/precompiles/werc20"
	"github.com/haqq-network/haqq/precompiles/werc20/testdata"
...
var _ = Describe("WISLM Extension -", func() {
	var (
		...
		WISLMOriginalContractAddr common.Address
		err                       error
		sender                    keyring.Key
		amount                    *big.Int
	...
	Context("Comparing to original WISLM contract", func() {
		BeforeEach(func() {
			WISLMOriginalContractAddr, err = s.factory.DeployContract(
				sender.Priv,
				evmtypes.EvmTxArgs{}, // NOTE: passing empty struct to use default values
				factory.ContractDeploymentData{
					Contract:        testdata.WISLMContract,
			...
			contractDataOriginal = ContractData{
				ownerPriv: sender.Priv,
				erc20Addr: WISLMOriginalContractAddr,
				erc20ABI:  testdata.WISLMContract.ABI,
			}
		})
	...
	Context("ERC20 specific functions", func() {
		When("querying name", func() {
			It("should return the correct name", func() {
				...
				Expect(err).ToNot(HaveOccurred(), "failed to unpack result")
				Expect(name).To(Equal("Wrapped ISLM"), "expected different name")
			})
		})
		...
		When("querying symbol", func() {
			It("should return the correct symbol", func() {
				...
				Expect(err).ToNot(HaveOccurred(), "failed to unpack result")
				Expect(symbol).To(Equal("WISLM"), "expected different symbol")
			})
		})

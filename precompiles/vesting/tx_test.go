...
var (
	...
	toAddr           = testutiltx.GenerateAddress()
	funderAddr       = testutiltx.GenerateAddress()
	diffFunderAddr   = testutiltx.GenerateAddress()
	...
	testCases := []struct {
		...
		},
		{
			...
			func() []interface{} {
				...
				err = haqqtestutil.FundAccount(s.ctx, s.app.BankKeeper, toAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, sdk.NewInt(100))))
				return []interface{}{
					s.address,
					toAddr,
					uint64(time.Now().Unix()),

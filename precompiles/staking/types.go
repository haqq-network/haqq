...
func (vo *ValidatorOutput) FromResponse(res *stakingtypes.QueryValidatorResponse) ValidatorOutput {
	...
	return ValidatorOutput{
		Validator: ValidatorInfo{
			...
			Status:          uint8(stakingtypes.BondStatus_value[res.Validator.Status.String()]), //nolint: gosec
	...
	for i, v := range res.Validators {
		...
		if err != nil {
			...
			vo.Validators[i] = ValidatorInfo{
				...
				Jailed:            v.Jailed,
				Status:            uint8(stakingtypes.BondStatus_value[v.Status.String()]), //nolint: gosec

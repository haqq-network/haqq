...
func (s *PrecompileTestSuite) setupVestingAccount(funder, vestAcc testkeyring.Key) *vestingtypes.ClawbackVestingAccount {
	...
	err := haqqtestutil.FundAccount(ctx, s.network.App.BankKeeper, vestAcc.AccAddr, vestingAmtTotal)
	Expect(err).To(BeNil())
	err = s.network.App.BankKeeper.SendCoins(ctx, vestAcc.AccAddr, funder.AccAddr, vestingAmtTotal)
	Expect(err).To(BeNil())
	Expect(s.network.NextBlock()).To(BeNil())

	vestingStart := ctx.BlockTime()
	// create a clawback vesting account
	msgConv := vestingtypes.NewMsgConvertIntoVestingAccount(
		funder.AccAddr,
		vestAcc.AccAddr,
		vestingStart,
	...
	_, err = s.factory.ExecuteCosmosTx(funder.Priv, cmnfactory.CosmosTxArgs{Msgs: []sdk.Msg{msgConv}})
	Expect(err).To(BeNil())
	Expect(s.network.NextBlock()).To(BeNil())

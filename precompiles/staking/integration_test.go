	...
	Describe("Calling precompile txs from a vesting account", func() {
		...
		Context("before first vesting period - all tokens locked and unvested", func() {
			...
			It("Should be able to delegate tokens not involved in vesting schedule", func() {
				...
				Expect(err).To(BeNil())
				Expect(*balRes.Balance).To(Equal(accountGasCoverage[0].Add(haqqtestutil.TestVestingSchedule.TotalVestingCoins[0]).Add(coinsToDelegate[0])))
				...
				Expect(err).To(BeNil())
				Expect(balRes.Balance.IsGTE(haqqtestutil.TestVestingSchedule.TotalVestingCoins[0])).To(BeTrue())
			})
		})
		...
		Context("after first vesting period and before lockup - some vested tokens, but still all locked", func() {
			BeforeEach(func() {
				...
				unlockedVested = clawbackAccount.GetVestedCoins(ctx.BlockTime())
			...
			It("Should be able to delegate locked vested tokens + free tokens (not in vesting schedule)", func() {
				...
				balRes, err := s.grpcHandler.GetBalance(vestAcc.Bytes(), s.bondDenom)
				Expect(err).To(BeNil())
				Expect(*balRes.Balance).To(Equal(accountGasCoverage[0].Add(haqqtestutil.TestVestingSchedule.TotalVestingCoins[0]).Add(amt[0])))

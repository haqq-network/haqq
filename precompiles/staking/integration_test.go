...
import (
	...
	compiledcontracts "github.com/haqq-network/haqq/contracts"
	...
	Describe("to edit validator", func() {
		...
		Context("when origin is equal to validator address", func() {
			It("should succeed", func() {
				...
				err := haqqtestutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, newAddr, 2e18)
...
var _ = Describe("Calling staking precompile via Solidity", func() {
	...
	BeforeEach(func() {
		...
		err = haqqtestutil.FundAccount(s.ctx, s.app.BankKeeper, contractTwoAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, testContractInitialBalance)))
		Expect(err).To(BeNil(), "error while funding the smart contract: %v", err)

		err = haqqtestutil.FundAccount(s.ctx, s.app.BankKeeper, stkReverterAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, testContractInitialBalance)))
		Expect(err).To(BeNil(), "error while funding the smart contract: %v", err)
	...
	Context("create a validator", func() {
		...
		BeforeEach(func() {
			...
			err := haqqtestutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, valAddr, 1e18)
			Expect(err).To(BeNil(), "error while funding account: %v", err)
	...
	Context("to edit a validator", func() {
		...
		BeforeEach(func() {
			...
			err := haqqtestutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, valAddr, 2e18)
			Expect(err).To(BeNil(), "error while funding account: %v", err)
			...
			Context("Calling the precompile from the StakingReverter contract", func() {
				...
				It("should revert the changes and NOT delegate - failed tx - max precompile calls reached", func() {
					callArgs := contracts.CallArgs{
						...
						MethodName:   "run",
						Args: []interface{}{
							big.NewInt(100), s.validators[0].GetOperator().String(),
						},
						GasPrice: gasPrice.BigInt(),
					}
...
var _ = Describe("Batching cosmos and eth interactions", func() {
	...
	BeforeEach(func() {
		...
		contractAddr, err = haqqtestutil.DeployContract(s.ctx, s.app, s.privKey, s.queryClientEVM, stakingCallerContract)

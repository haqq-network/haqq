...
import (
	...
	"github.com/haqq-network/haqq/cmd/haqqd/opendb"
	"github.com/haqq-network/haqq/indexer"
	ethdebug "github.com/haqq-network/haqq/rpc/namespaces/ethereum/debug"
...
func startStandAlone(svrCtx *server.Context, opts StartOptions) error {
	...
	_, err = startTelemetry(srvConfig)
	if err != nil {
		return err
	}
...
func startInProcess(svrCtx *server.Context, clientCtx client.Context, opts StartOptions) (err error) {
	...
	if gRPCOnly {
		...
		srvConfig.GRPC.Enable = true
		srvConfig.JSONRPC.EnableIndexer = false
	} else {
	...
	if (srvConfig.API.Enable || srvConfig.GRPC.Enable || srvConfig.JSONRPC.Enable || srvConfig.JSONRPC.EnableIndexer) && tmNode != nil {
		clientCtx = clientCtx.WithClient(local.New(tmNode))

		app.RegisterTxService(clientCtx)
		app.RegisterTendermintService(clientCtx)
		app.RegisterNodeService(clientCtx, srvConfig.Config)
	}

	metrics, err := startTelemetry(srvConfig)
	if err != nil {
		return err
	}
	...
	if srvConfig.JSONRPC.Enable && svrCtx.Viper.GetBool(srvflags.JSONRPCEnableMetrics) {
		ethmetricsexp.Setup(srvConfig.JSONRPC.MetricsAddress)
	}

	var idxer haqqtypes.EVMTxIndexer
	if srvConfig.JSONRPC.EnableIndexer {
	...
	if srvConfig.API.Enable || srvConfig.JSONRPC.Enable {
	...
	grpcSrv, clientCtx, err := startGrpcServer(ctx, svrCtx, clientCtx, g, srvConfig.GRPC, app)
	if err != nil {
		return err
	}
	if grpcSrv != nil {
	...
	apiSrv := startAPIServer(ctx, svrCtx, clientCtx, g, srvConfig.Config, app, grpcSrv, metrics)
	if apiSrv != nil {
		defer apiSrv.Close()
	}

	clientCtx, httpSrv, httpSrvDone, err := startJSONRPCServer(svrCtx, clientCtx, g, srvConfig, genDocProvider, cfg.RPC.ListenAddress, idxer)
	if httpSrv != nil {
		defer func() {
	...
	if err := startRosettaServer(svrCtx, clientCtx, g, srvConfig); err != nil {
		return err
	}
	// wait for signal capture and gracefully return
	// we are guaranteed to be waiting for the "ListenForQuitSignals" goroutine.

...
# The path to the main level of the Haqq repository.
...
def is_haqq_repo(path: Path) -> bool:
    """
    This function checks if the given path is the root of the Haqq repository,
    ...
    with open(path / "go.mod", "r", encoding="utf-8") as go_mod:
        while True:
            ...
            if "module github.com/haqq-network/haqq" in line:
                return True
...
if __name__ == "__main__":
    if not is_haqq_repo(REPO_PATH):
        raise ValueError(
            "This script should only be executed "
            + "in the Haqq repository."

...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(haqqtypes.AttoEvmos, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(haqqtypes.AttoEvmos, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(haqqtypes.AttoEvmos)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, haqqtypes.AttoEvmos, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}

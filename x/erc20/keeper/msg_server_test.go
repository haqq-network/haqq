...
func (suite *KeeperTestSuite) TestConvertERC20NativeERC20() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				acc := suite.network.App.AccountKeeper.GetAccount(suite.network.GetContext(), types.ModuleAddress.Bytes())
				suite.network.App.AccountKeeper.RemoveAccount(suite.network.GetContext(), acc)
			},
			contractMinterBurner,
			false,
			false,

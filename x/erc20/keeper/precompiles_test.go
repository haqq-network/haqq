...
func (suite *KeeperTestSuite) TestGetERC20PrecompileInstance() {
	...
	nonExistentTokenHexAddr := "0x8FA78CEB7F04118Ec6d06AaC37Ca854691d8e963" //nolint:gosec
	newTokenDenom := "test"
	...
	testCases := []struct {
	...
	}{
		{
			...
			common.HexToAddress(nonExistentTokenHexAddr),
			false,
			false,
			"",
		},
		{
			...
			func() {
				params := types.DefaultParams()
				params.NativePrecompiles = []string{newTokenHexAddr, nonExistentTokenHexAddr}
				err := suite.network.App.Erc20Keeper.SetParams(ctx, params)
				suite.Require().NoError(err)
			},
			common.HexToAddress(nonExistentTokenHexAddr),

...
var _ = Describe("Convert receiving IBC to Erc20", Ordered, func() {
	...
	Describe("Performing recovery with registered coin", func() {
		...
		It("should recover and not convert uosmo to tokens", func() {
			...
			s.SendAndReceiveMessage(s.pathOsmosisHaqq, s.IBCOsmosisChain, "uosmo", amount, sender, receiver, 1, "")
			timeout := uint64(s.HaqqChain.GetContext().BlockTime().Add(time.Hour * 4).Add(time.Second * -20).UnixNano())
			err := s.pathOsmosisHaqq.RelayPacket(CreatePacket("10", "transfer/channel-0/uosmo", sender, receiver, "transfer", "channel-0", "transfer", "channel-0", 1, timeout))
			s.Require().NoError(err)

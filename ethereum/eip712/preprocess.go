...
func PreprocessLedgerTx(chainID string, keyType cosmoskr.KeyType, txBuilder client.TxBuilder) error {
	...
	chainIDInt, err := haqqtypes.ParseChainID(chainID)
	if err != nil {
	...
	option, err = codectypes.NewAnyWithValue(&haqqtypes.ExtensionOptionsWeb3Tx{
	...
	// (Regardless of input signMode, Haqq requires Amino signature type for Ledger)

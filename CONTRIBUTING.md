...
1. Either [open](https://github.com/haqq-network/haqq/issues/new/choose)
   or [find](https://github.com/haqq-network/haqq/issues) an issue you have identified and would like to contribute to
   resolving.
...
3. If you would like to contribute:
    ...
    4. Follow standard Github best practices:
        1. Fork the repo
        2. Branch from the HEAD of `development`(For core developers working within the haqq repo, to ensure a
...
some [good first issues](https://github.com/haqq-network/haqq/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Evmos, please create
an [ADR](https://github.com/haqq-network/haqq/blob/main/docs/architecture/README.md) so further discussions can be
...
at `https://github.com/haqq-network/haqq`, the code should never exist
at `$GOPATH/src/github.com/haqq-network/haqq`. Instead, we use `git remote` to add the fork as a new remote for the
original repo,`$GOPATH/src/github.com/haqq-network/haqq`, and do all the work there.
...
We use [Go 1.19](https://github.com/golang/go/wiki/Modules) Modules to manage dependency versions.
